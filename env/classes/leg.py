...
# Func
from env.func.calculations import calc_circle_coordinates, calc_servo_angles
...
class Leg:
    ...
    # @validate_types
    def __init__(
        self, *, leg_configurations: LegConfigDict, leg: str, stop_event: StopEvent
    ) -> None:
        ...
        if leg not in ["rf", "lf", "rb", "lb"]:
            raise ValueError(
                f"Invalid leg: {leg}. Must be one of 'rf', 'lf', 'rb', 'lb'."
            )
        ...
        self.current_position: Coordinate = Coordinate(
            0.0, 0.0, 0.0
        )  # Initialize current position to (0, 0, 0) --> Default position
        self.all_servos: tuple[SServo, SServo, SServo] = (
            self.thigh,
            self.lower_leg,
            self.side_axis,
        )
    ...
    def set_to_normal_position(
        ...
        """
        self.set_to_coordinate(
            coordinate=Coordinate(x=0.0, y=0.0, z=0.0), duration_s=duration_s
        )
    ...
    def set_to_coordinate(self, coordinate: Coordinate, duration_s: float) -> None:
        ...
        adjusted_coordinate: Coordinate = (
            coordinate * config.coord_multiplier
        )  # Adjust coordinate to the servo coordinate system
        angles: dict[Literal["thigh", "lower-leg", "side-axis"], int] = (
            calc_servo_angles(coordinate=adjusted_coordinate)
        )
    ...
    @validate_types
    def set_circle(
        self, step_width: float, angle: int, max_points: int, duration: float
    ) -> None:
        ...
        """
        coords: list[Coordinate] = calc_circle_coordinates(
            step_width=step_width, angle=angle, max_points=max_points
        )
    ...
    def start_circle(self) -> None:
        ...
        """
        if self.circle_thread is None:
            raise NoThreadError("No circle thread set!")
